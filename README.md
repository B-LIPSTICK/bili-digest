...
3. 「字幕」页：点时间戳跳转，切换原文 / 译文 / 双语，悬停某句点「解释」；切到译文不会自动翻译，点右上角「翻译」按钮才触发（结果会缓存复用），右上角还能复制全文或导出 Markdown；
4. 「概览」页：生成内容概要、章节、要点和金句，金句可复制或直接存为笔记，右上角可单独导出概览；
5. 记笔记：看视频时点右上角「记笔记」或按 N，AI 会把刚才那句话整理干净并自动保存；也可以在「笔记」页手写后点「AI 润色」；

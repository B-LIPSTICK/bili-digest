...
/**
 * 去掉 Base URL 末尾斜杠后拼接 /models（拉取可用模型列表）。
 */
...
/**
 * 断言 baseUrl 使用安全协议，防止请求被重定向到内网或非 HTTPS 地址。
 */
function assertSafeBaseUrl(baseUrl) {
  ...
  try {
    ...
    throw new Error("接口地址不合法");
  }
  if (parsed.protocol !== "https:" && !(parsed.protocol === "http:" && parsed.hostname === "localhost")) {
    throw new Error("接口地址必须使用 HTTPS（本地服务可使用 http://localhost）");
  }
}
